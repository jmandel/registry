...
for url in $urls; do
  ...
  jq '.repository = "https://github.com/jmandel/registry" | .name |= "@jmandel/" + . | if has("dependencies") then .dependencies |= with_entries(if .key | startswith("hl7.") then .key |= "@jmandel/" + . else . end) else . end' package/package.json > package/package_modified.json
